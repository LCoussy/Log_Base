...
def batchOpen(dir_path):
    ...
    if os.path.isdir(dir_path):
        for filename in os.listdir(dir_path):
            ...
            # If the current path is a directory, process it recursively
            if os.path.isdir(full_path):
                filespath.extend(batchOpen(full_path))
            else:
                filespath.append(full_path)  # Add the file path to the list
    else:
        filespath.append(os.path.realpath(dir_path))  # Add single file path to the list
